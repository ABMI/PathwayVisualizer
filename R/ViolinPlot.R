...
violinPlot<-function(connectionDetails,
  ...
  plotData<-collapsedCohort %>% arrange(cohortName,dateDiff) %>% mutate(category = ifelse(dateDiff<1,'d0',ifelse(dateDiff<=7,'d1-d7',ifelse(dateDiff<=14,'d8-d14',ifelse(dateDiff<=21,'d15-d21',ifelse(dateDiff<=30,'-d30','>d30'))))))
  plotData$category <- factor(plotData$category,levels = c('d0','d1-d7','d8-d14','d15-d21','-d30','>d30'))
  # plot
