...
plotRaw_3 <- function(connectionDetails,
  ...
  minimunIndexId <- unique(eventAndTarget %>% arrange(subjectId,cohortStartDate) %>% group_by(subjectId) %>% mutate(line = row_number()) %>% subset(line >= maximumPathLength + 1) %>% select(subjectId) %>% ungroup())
  ...
  treatmentRatio <- data.table::rbindlist(lapply(1:maximumPathLength,function(x){
    eventAndTarget %>% subset(rowNumber==x) %>% group_by(nameOfConcept) %>% summarise(n=n()) %>% mutate(ratio=round(n/sum(n)*100,1))
    }
    )
    )

  # Minimum Cell Count
  treatmentRatio <- treatmentRatio %>% subset(n >= minimumCellCount)
